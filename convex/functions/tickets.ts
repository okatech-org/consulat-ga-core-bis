...
export const get = authQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // Only the ticket owner OR a superadmin can view it
    const isOwner = ticket.userId === ctx.user._id;
    const isSuperadminVal = await requireSuperadmin(ctx).then(() => true).catch(() => false);
    
    if (!isOwner && !isSuperadminVal) {
...
export const addMessage = authMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const newMessage = {
      ...
      isStaff: !!isSuperadminVal && !isOwner,
