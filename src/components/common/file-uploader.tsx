...
export function FileUploader({
  ...
  }, [generateUploadUrl, createDocument, docType, onUploadComplete, onUploadError])
      ...
      {uploadingFiles.length > 0 && (
          <div className="space-y-2">
              {uploadingFiles.map((file, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-2 border rounded-md bg-muted/20">
                      <Loader2 className="h-4 w-4 animate-spin text-primary" />
                      <span className="text-sm flex-1 truncate">{file.name}</span>
                      {/* Progress bar could be added here if we had XHR progress */}
                  </div>
              ))}
          </div>
      )}
