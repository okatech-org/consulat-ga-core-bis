...
import { MapPin, Phone, Clock, Building2, ArrowRight, ExternalLink } from 'lucide-react'
...
export function NearbyOrgs({ maxItems = 3, showTitle = true }: NearbyOrgsProps) {
          ...
          <div>
            ...
            {!isLoading && countryName && (
              <p className="text-sm text-muted-foreground mt-1">
                {t('nearbyOrgs.inCountry', 'En {{country}}', { country: countryName })}
              </p>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        {isLoading ? (
          ...
          orgs?.slice(0, maxItems).map((org) => {
            ...
            const orgCountryName = countryNames[org.address.country] || org.address.country
              ...
              >
                {isPrimary && (
                  <Badge className="absolute top-4 right-4">
                    {org.type === OrgType.EMBASSY ? t('consulates.embassy', 'Ambassade') : t('consulates.headquarters', 'Siège')}
                  </Badge>
                )}
