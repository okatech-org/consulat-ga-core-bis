...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <span className={cn("text-xs font-medium px-2 py-0.5 rounded-full", config.class)}>
      {config.label}
    </span>
  )
}
          ...
          {isEvent && post.eventStartAt && (
            <div className="flex flex-wrap gap-3 text-sm text-muted-foreground mb-4">
              <div className="flex items-center gap-1">
                ...
                <span>{format(new Date(post.eventStartAt), "d MMM yyyy", { locale: fr })}</span>
              </div>
              {post.eventLocation && (
                <div className="flex items-center gap-1">
                  <MapPin className="h-4 w-4" />
                  <span className="truncate max-w-[200px]">{post.eventLocation}</span>
                </div>
              )}
            </div>
          )}
...
export function NewsSection() {
  ...
  const { data: posts } = useConvexQuery(api.functions.posts.getLatest, { limit: 4 })
