...
const addressSchema = z.object({
  ...
  country: z.enum(CountryCode, { message: "errors.profile.addresses.country.invalid" }),
})
...
const emergencyContactSchema = z.object({
  ...
  email: z.email({ message: "errors.profile.contacts.emergency.email.invalid" }).optional(),
  relationship: z.enum(FamilyLink, { message: "errors.profile.contacts.emergency.relationship.invalid" }),
})
...
export const profileFormSchema = z.object({
  identity: z.object({
    ...
    birthCountry: z.enum(CountryCode, { message: "errors.profile.identity.birthCountry.invalid" }).optional(),
    gender: z.enum(Gender, { message: "errors.profile.identity.gender.invalid" }).optional(),
    nationality: z.enum(CountryCode, { message: "errors.profile.identity.nationality.invalid" }).optional(),
    nationalityAcquisition: z.enum(NationalityAcquisition, { message: "errors.profile.identity.nationalityAcquisition.invalid" }).optional(),
  }),
  ...
  contacts: z.object({
    ...
    email: z.email({ message: "errors.profile.contacts.email.invalid" }).optional(),
    emergencyResidence: emergencyContactSchema.optional(),
    emergencyHomeland: emergencyContactSchema.optional(),
  }),
  family: z.object({
    maritalStatus: z.enum(MaritalStatus, { message: "errors.profile.family.maritalStatus.invalid" }).optional(),
    ...
    const requiresSpouse = data.maritalStatus && [MaritalStatus.Married, MaritalStatus.CivilUnion].includes(data.maritalStatus as MaritalStatus)
    
    if (requiresSpouse) {
      // Si le statut nécessite un conjoint, valider que les champs sont remplis
      if (!data.spouse?.firstName || data.spouse.firstName.trim().length < 2) {
        ctx.addIssue({
          code: "custom",
