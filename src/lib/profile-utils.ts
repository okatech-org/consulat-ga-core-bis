...
export function getChangedFields(
  ...
  if (formData.identity) {
    ...
    if (isDifferent(formData.identity.birthPlace, originalIdentity?.birthPlace)) {
      changedIdentity.birthPlace = formData.identity.birthPlace
      hasChanges = true
    }
    if (isDifferent(formData.identity.birthCountry, originalIdentity?.birthCountry)) {
    ...
    if (isDifferent(formData.identity.nationality, originalIdentity?.nationality)) {
    ...
    if (isDifferent(
      ...
      changedIdentity.nationalityAcquisition = formData.identity.nationalityAcquisition
      hasChanges = true
    }
  ...
  if (formData.passportInfo || originalProfile.passportInfo) {
    ...
    if (!originalPassport && formPassport) {
      ...
      if (isDifferent(formPassport.issuingAuthority, originalPassport.issuingAuthority)) {
  ...
  if (formData.family) {
    ...
    if (isDifferent(formData.family.maritalStatus, originalFamily?.maritalStatus)) {
...
export function transformFormDataToPayload(formData: Partial<ProfileFormValues>): any {
  ...
  if (formData.passportInfo) {
    payload.passportInfo = {
      ...
      issueDate: formData.passportInfo.issueDate instanceof Date
        ? formData.passportInfo.issueDate.getTime()
        : formData.passportInfo.issueDate,
      expiryDate: formData.passportInfo.expiryDate instanceof Date
