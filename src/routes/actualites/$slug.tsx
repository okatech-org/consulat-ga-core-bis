...
function CategoryBadge({ category }: { category: string }) {
  const config = {
    ...
    communique: { label: "Communiqué officiel", class: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300" },
  }[category] ?? { label: category, class: "bg-gray-100 text-gray-800" }

  return (
    <span className={cn("text-sm font-medium px-3 py-1 rounded-full", config.class)}>
      {config.label}
    </span>
  )
}
...
function PostDetailPage() {
  ...
  const { data: post, isLoading } = useConvexQuery(api.functions.posts.getBySlug, { slug })
  ...
  const isCommunique = post.category === PostCategory.Communique
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {post.eventStartAt && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="font-medium">{t("news.event.date", "Date")}</div>
                      <div className="text-sm text-muted-foreground">
                        {format(new Date(post.eventStartAt), "EEEE d MMMM yyyy 'à' HH:mm", { locale: fr })}
                        {post.eventEndAt && (
                          <>
                            <br />
                            jusqu'au {format(new Date(post.eventEndAt), "d MMMM yyyy 'à' HH:mm", { locale: fr })}
                          </>
                        )}
                      </div>
                    </div>
                ...
                {post.eventLocation && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="font-medium">{t("news.event.location", "Lieu")}</div>
                      <div className="text-sm text-muted-foreground">{post.eventLocation}</div>
                    </div>
                  </div>
                )}
              </div>
              {post.eventTicketUrl && (
                <div className="mt-4 pt-4 border-t border-amber-200 dark:border-amber-800">
                  <Button asChild className="w-full sm:w-auto">
                    <a href={post.eventTicketUrl} target="_blank" rel="noopener noreferrer">
            ...
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-6 mb-8">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="font-medium">{t("news.communique.document", "Document officiel")}</div>
                  <div className="text-sm text-muted-foreground">
                    {t("news.communique.downloadHint", "Téléchargez le document officiel au format PDF")}
                  </div>
                </div>
                <Button asChild variant="outline">
                  <a href={post.documentUrl} target="_blank" rel="noopener noreferrer" download>
