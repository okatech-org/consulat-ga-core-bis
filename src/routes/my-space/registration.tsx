...
import { Loader2, CheckCircle2, Clock, MapPin, Building2, AlertTriangle, ArrowLeft, ArrowRight, Phone, Mail, Globe, FileText, User, Home, Info } from "lucide-react"
...
// Configurable required fields for profile completion
...
function RegistrationPage() {
  ...
  const { data: registrationService, isPending: servicePending } = useConvexQuery(
  ...
  const missingFields = useMemo(() => {
    ...
    if (REQUIRED_PROFILE_FIELDS.identity.firstName && !profile.identity?.firstName) {
      missing.push(t("profile.identity.firstName", "Prénom"))
    }
    if (REQUIRED_PROFILE_FIELDS.identity.lastName && !profile.identity?.lastName) {
      missing.push(t("profile.identity.lastName", "Nom"))
    }
    if (REQUIRED_PROFILE_FIELDS.identity.birthDate && !profile.identity?.birthDate) {
      missing.push(t("profile.identity.birthDate", "Date de naissance"))
    }
    if (REQUIRED_PROFILE_FIELDS.identity.birthPlace && !profile.identity?.birthPlace) {
      missing.push(t("profile.identity.birthPlace", "Lieu de naissance"))
    }

    // Check address
    if (REQUIRED_PROFILE_FIELDS.addresses.residence && !profile.addresses?.residence) {
    ...
    if (REQUIRED_PROFILE_FIELDS.documents.passport && (!docs.passport || docs.passport.length === 0)) {
      missing.push(t("profile.documents.passport", "Passeport"))
    }
    if (REQUIRED_PROFILE_FIELDS.documents.nationalId && (!docs.nationalId || docs.nationalId.length === 0)) {
      missing.push(t("profile.documents.nationalId", "Carte Nationale d'Identité"))
    }
    if (REQUIRED_PROFILE_FIELDS.documents.photo && (!docs.photo || docs.photo.length === 0)) {
    ...
    return (
      <div className="p-4">
        <Alert variant="destructive">
          ...
          <AlertTitle>{t("registration.accessRestricted", "Accès Restreint")}</AlertTitle>
          <AlertDescription>
            {t("registration.nationalsOnly", "Cette page est réservée aux ressortissants nationaux.")}
          </AlertDescription>
        </Alert>
  ...
  const registeredOrg = existingRegistration ? allOrgs?.find((o: { _id: Id<"orgs"> }) => o._id === existingRegistration.orgId) : null
  ...
  const selectedOrg = jurisdictionOrgs?.find((o: { _id: string }) => o._id === selectedOrgId)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(error.message || t("registration.error", "Erreur lors de l'envoi"))
    }
  }

  const steps = [
    { id: "select-org", label: t("registration.steps.selectOrg", "Organismes") },
    { id: "check-service", label: t("registration.steps.checkService", "Disponibilité") },
    { id: "check-profile", label: t("registration.steps.checkProfile", "Profil") },
      ...
      {currentStep === "select-org" && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {residenceCountry 
                ? t("registration.selectOrgDesc", "Organismes disponibles pour votre pays de résidence")
                : t("registration.noResidenceCountry", "Veuillez d'abord renseigner votre pays de résidence dans votre profil.")}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {!residenceCountry ? (
              <Alert>
                ...
                <AlertTitle>{t("registration.missingResidence.title", "Pays de résidence requis")}</AlertTitle>
                <AlertDescription>
                  {t("registration.missingResidence.description", "Pour afficher les consulats disponibles, vous devez renseigner votre adresse de résidence dans votre profil.")}
                </AlertDescription>
              </Alert>
            ) : orgsPending ? (
              ...
              <Alert>
                <AlertTriangle className="h-4 w-4" />
                <AlertTitle>{t("registration.noOrgs.title", "Aucun organisme disponible")}</AlertTitle>
                <AlertDescription>
                  {t("registration.noOrgs.description", "Il n'y a pas d'organisme consulaire couvrant votre pays de résidence pour le moment. Veuillez contacter le ministère des affaires étrangères pour plus d'informations.")}
                </AlertDescription>
              </Alert>
            ) : (
              <div className="space-y-2">
                ...
                <Select value={selectedOrgId} onValueChange={setSelectedOrgId}>
                  ...
                  <SelectContent>
                    {jurisdictionOrgs.map((org: { _id: string; name: string; type: string; address: { city: string; country: string } }) => (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t("registration.serviceAvailability", "Disponibilité du service")}
            </CardTitle>
            <CardDescription>
              {selectedOrg?.name}
            </CardDescription>
            ...
            {servicePending ? (
              ...
              <Alert className="border-green-500/50 bg-green-50 dark:bg-green-950/20">
                ...
                <AlertTitle className="text-green-700 dark:text-green-400">
                  {t("registration.serviceAvailable.title", "Service disponible en ligne")}
                </AlertTitle>
                <AlertDescription className="text-green-600 dark:text-green-400">
                  {t("registration.serviceAvailable.description", "Vous pouvez faire votre demande d'immatriculation en ligne. Votre dossier sera traité par l'organisme sélectionné.")}
                </AlertDescription>
              </Alert>
            ) : (
              <div className="space-y-4">
                <Alert variant="destructive">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertTitle>{t("registration.serviceUnavailable.title", "Service non disponible en ligne")}</AlertTitle>
                  <AlertDescription>
                    {t("registration.serviceUnavailable.description", "Cet organisme ne propose pas l'immatriculation en ligne. Veuillez vous rendre sur place ou prendre rendez-vous.")}
                  </AlertDescription>
                </Alert>
                    ...
                    <CardContent className="space-y-2 text-sm">
                      {selectedOrg.address && (
                        <div className="flex items-start gap-2">
                          ...
                          <span>{selectedOrg.address.street}, {selectedOrg.address.postalCode} {selectedOrg.address.city}</span>
                        </div>
                      )}
                      {selectedOrg.phone && (
                        <div className="flex items-center gap-2">
                          <Phone className="h-4 w-4 text-muted-foreground" />
                          <a href={`tel:${selectedOrg.phone}`} className="text-primary hover:underline">{selectedOrg.phone}</a>
                        </div>
                      )}
                      {selectedOrg.email && (
                        <div className="flex items-center gap-2">
                          <Mail className="h-4 w-4 text-muted-foreground" />
                          <a href={`mailto:${selectedOrg.email}`} className="text-primary hover:underline">{selectedOrg.email}</a>
                        </div>
                      )}
                      {selectedOrg.website && (
                        <div className="flex items-center gap-2">
                          <Globe className="h-4 w-4 text-muted-foreground" />
                          <a href={selectedOrg.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{selectedOrg.website}</a>
                        </div>
                      )}
                    </CardContent>
                  </Card>
      ...
      {currentStep === "check-profile" && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t("registration.profileCheckDesc", "Votre profil doit être complet pour soumettre une demande.")}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {isProfileComplete ? (
              <Alert className="border-green-500/50 bg-green-50 dark:bg-green-950/20">
                ...
                <AlertDescription className="text-green-600 dark:text-green-400">
                  {t("registration.profileComplete.description", "Votre profil contient toutes les informations requises.")}
                </AlertDescription>
              </Alert>
            ) : (
              <div className="space-y-4">
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    <p className="mb-2">{t("registration.missingDocs.description", "Veuillez compléter les informations suivantes:")}</p>
      ...
      {currentStep === "submit" && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t("registration.submitDesc", "Vérifiez les informations avant de soumettre.")}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
