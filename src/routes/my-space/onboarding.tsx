	...
	const handleSubmit = async () => {
		...
		try {
			await createProfile({
				...
				identity: {
					nationality:
						nationality === "national" ? CountryCode.GA : CountryCode.FR,
				},
			});
			...
			<Card className="w-full max-w-md shadow-lg animate-in fade-in zoom-in-95">
				<CardHeader className="text-center space-y-2 pb-2">
					...
					<CardTitle className="text-2xl">
						{t("onboarding.title", "Bienvenue!")}
					</CardTitle>
					<CardDescription className="text-base">
						{t(
							"onboarding.subtitle",
							"Pour commencer, quelle est votre situation?",
						)}
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-6 pt-4">
					<RadioGroup
						...
						onValueChange={(v: string) =>
							setNationality(v as "national" | "foreigner")
						}
						className="space-y-3"
					>
						<Label
						...
						>
							...
							<div className="flex-1">
								<div className="font-medium">
									{t("onboarding.national.title", "Je suis Gabonais(e)")}
								</div>
								<div className="text-sm text-muted-foreground">
									{t(
										"onboarding.national.desc",
										"Accédez à l'immatriculation consulaire et aux services dédiés.",
									)}
								</div>
							</div>
						...
						>
							...
							<div className="flex-1">
								<div className="font-medium">
									{t("onboarding.foreigner.title", "Je suis étranger")}
								</div>
								<div className="text-sm text-muted-foreground">
									{t(
										"onboarding.foreigner.desc",
										"Effectuez vos demandes de visa et autres formalités.",
									)}
								</div>
							</div>
