...
import { ArrowLeft, Newspaper, CalendarDays, Megaphone, Upload, Calendar, MapPin, Ticket, FileText, Loader2 } from "lucide-react"
...
function EditPostPage() {
  ...
  const post = useQuery(api.functions.posts.getById, { postId: postId as Id<"posts"> })
  ...
  const [category, setCategory] = useState<typeof PostCategory[keyof typeof PostCategory]>(PostCategory.News)
  ...
  // Cover image
  const [coverImageStorageId, setCoverImageStorageId] = useState<Id<"_storage"> | undefined>()
  ...
  const handleCoverImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(t("dashboard.posts.uploadError", "Erreur lors du téléchargement"))
    }
  }
  ...
  const handleDocumentUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(t("dashboard.posts.documentUploaded", "Document téléchargé"))
    } catch {
      toast.error(t("dashboard.posts.uploadError", "Erreur lors du téléchargement"))
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !slug || !excerpt || !content) {
      toast.error(t("dashboard.posts.requiredFields", "Veuillez remplir tous les champs obligatoires"))
      return
    }

    if (category === PostCategory.Communique && !documentStorageId) {
      toast.error(t("dashboard.posts.documentRequired", "Un document PDF est obligatoire pour les communiqués"))
      return
    }
    ...
    try {
      await update({
        ...
        coverImageStorageId,
        // Event fields
        eventStartAt: eventStartAt ? new Date(eventStartAt).getTime() : undefined,
    ...
    <div className="flex flex-1 flex-col gap-4 p-4">
      ...
      <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>{t("dashboard.posts.form.content", "Contenu")}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="title">{t("dashboard.posts.form.title", "Titre")} *</Label>
                <Input
                  id="title"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder={t("dashboard.posts.form.titlePlaceholder", "Titre de l'article")}
                />
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="excerpt">{t("dashboard.posts.form.excerpt", "Résumé")} *</Label>
                <Textarea
                  id="excerpt"
                  value={excerpt}
                  onChange={(e) => setExcerpt(e.target.value)}
                  placeholder={t("dashboard.posts.form.excerptPlaceholder", "Un court résumé...")}
                  rows={3}
                />
              </div>

              <div className="space-y-2">
                <Label>{t("dashboard.posts.form.body", "Corps de l'article")} *</Label>
                <RichTextEditor
                  content={content}
                  onChange={setContent}
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {t("dashboard.posts.form.eventDetails", "Détails de l'événement")}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder={t("dashboard.posts.form.eventLocationPlaceholder", "Adresse ou nom du lieu")}
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="eventTicketUrl">
                    ...
                    {t("dashboard.posts.form.eventTicket", "Lien billetterie / inscription")}
                  </Label>
                  <Input
                    id="eventTicketUrl"
                    type="url"
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {t("dashboard.posts.form.communiqueDetails", "Document officiel")}
                </CardTitle>
                <CardDescription>
                  {t("dashboard.posts.form.communiqueHint", "Téléchargez le document PDF officiel (obligatoire)")}
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {documentName ? (
                    ...
                    <label htmlFor="document-upload" className="cursor-pointer">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {t("dashboard.posts.form.uploadDocument", "Cliquez pour télécharger un PDF")}
                      </p>
                    </label>
                  )}
                </div>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>{t("dashboard.posts.form.settings", "Paramètres")}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>{t("dashboard.posts.form.category", "Catégorie")} *</Label>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value={PostCategory.Communique}>
                      <div className="flex items-center gap-2">
                        ...
                        {t("dashboard.posts.category.communique", "Communiqué officiel")}
                      </div>
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-2">
                <Label>{t("dashboard.posts.form.coverImage", "Image de couverture")}</Label>
              ...
              <div className="p-3 bg-muted rounded-lg">
                <p className="text-sm">
                  <span className="font-medium">{t("dashboard.posts.form.status", "Statut")}:</span>{" "}
                  {post.status === PostStatus.Published ? (
                    <span className="text-green-600">{t("dashboard.posts.statusPublished", "Publié")}</span>
                  ) : (
                    <span className="text-muted-foreground">{t("dashboard.posts.statusDraft", "Brouillon")}</span>
                  )}
                </p>
              </div>
            </CardContent>
