...
import { ArrowLeft, Newspaper, CalendarDays, Megaphone, Upload, FileText, Loader2 } from "lucide-react"
...
function AdminEditPostPage() {
  ...
  useEffect(() => {
    if (post) {
      ...
      if (post.eventStartAt) setEventStartAt(new Date(post.eventStartAt).toISOString().slice(0, 16))
      if (post.eventEndAt) setEventEndAt(new Date(post.eventEndAt).toISOString().slice(0, 16))
  ...
  const handleTogglePublish = async () => {
    if (!post) return
    const newStatus = post.status === PostStatus.Published ? PostStatus.Draft : PostStatus.Published
    try {
      await setStatus({ postId: postId as Id<"posts">, status: newStatus })
      toast.success(newStatus === PostStatus.Published ? "Article publié" : "Article dépublié")
    } catch (err: any) {
      toast.error(err.message || "Une erreur est survenue")
    }
  }
    ...
    <div className="flex flex-1 flex-col gap-4 p-4">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold tracking-tight">Modifier la publication</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>Corps *</Label>
                <RichTextEditor content={content} onChange={setContent} className="min-h-[300px]" />
              </div>
            </CardContent>
          </Card>
          ...
          {isEvent && (
            <Card>
              <CardHeader><CardTitle className="flex items-center gap-2"><CalendarDays className="h-5 w-5" />Événement</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="url" value={eventTicketUrl} onChange={(e) => setEventTicketUrl(e.target.value)} />
                </div>
              </CardContent>
            </Card>
          )}
          ...
          {isCommunique && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><Megaphone className="h-5 w-5" />Document</CardTitle>
                <CardDescription>PDF obligatoire</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="border-2 border-dashed rounded-lg p-6 text-center">
                  <input type="file" accept="application/pdf" onChange={handleDocumentUpload} className="hidden" id="document-upload" />
                  {documentName ? (
                    <div className="flex items-center justify-center gap-3">
                      <FileText className="h-8 w-8 text-emerald-600" />
                      <div>
                        <p className="font-medium">{documentName}</p>
                        <Button type="button" variant="link" size="sm" onClick={() => document.getElementById("document-upload")?.click()}>Changer</Button>
                      </div>
                    </div>
                  ) : (
                    <label htmlFor="document-upload" className="cursor-pointer">
                      <Upload className="h-10 w-10 mx-auto mb-2 text-muted-foreground" />
                      <p className="text-sm text-muted-foreground">Télécharger PDF</p>
                    </label>
                  )}
                </div>
              </CardContent>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Select value={category} onValueChange={(v) => setCategory(v as any)}>
                  ...
                  <SelectContent>
                    <SelectItem value={PostCategory.News}><Newspaper className="inline mr-2 h-4 w-4" />Actualité</SelectItem>
                    <SelectItem value={PostCategory.Event}><CalendarDays className="inline mr-2 h-4 w-4" />Événement</SelectItem>
                    <SelectItem value={PostCategory.Communique}><Megaphone className="inline mr-2 h-4 w-4" />Communiqué</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                ...
                <div className="border-2 border-dashed rounded-lg p-4 text-center">
                  ...
                  {coverImagePreview ? (
                    <div>
                      <img src={coverImagePreview} alt="" className="w-full h-32 object-cover rounded mb-2" />
                      <Button type="button" variant="link" size="sm" onClick={() => document.getElementById("cover-upload")?.click()}>Changer</Button>
                    </div>
                  ) : (
